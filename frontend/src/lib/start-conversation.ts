...
export async function startConversation(content: string, files: File[], model: string, onCreated: (conversation: Conversation) => void, projectId?: string, contextIds: string[] = []) {
  ...
  const conversation = await api<Conversation>("/api/v1/conversations", {
    method: "POST",
    body: JSON.stringify({ project_id: projectId, agent_slug: "lester", model_deployment_id: model, title: Array.from(content.trim() || files[0].name).slice(0, 60).join("") }),
  });
