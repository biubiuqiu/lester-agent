...
export async function startConversation(content: string, files: File[], model: string, onCreated: (conversation: Conversation) => void, projectId?: string, contextIds: string[] = [], agentSlug = "lester") {
  ...
  const conversation = await api<Conversation>("/api/v1/conversations", {
    method: "POST",
    body: JSON.stringify({ project_id: projectId, agent_slug: agentSlug, model_deployment_id: model, title: Array.from(content.trim() || files[0].name).slice(0, 60).join("") }),
  });
  ...
  await api<{ run_id: string }>(`/api/v1/conversations/${conversation.id}/messages`, {
    method: "POST", body: JSON.stringify({ content: content.trim(), context_ids: contextIds, attachment_ids: attachments.map((file) => file.id) }),
  });
  return conversation;
}
