...
import { Bot, Check, ChevronDown, FileText, Folder, GripVertical, Maximize2, Menu, Minimize2, PanelLeftClose, PanelLeftOpen, Paperclip, Plus, Send, Square, TerminalSquare, Wrench, X } from "lucide-react";
...
const agentName = (slug: string, savedName?: string) => savedName || agents.find((agent) => agent.slug === slug)?.name || "Agent";
...
export function Workspace({ conversationId, projectId, initialAgentSlug }: { conversationId?: string; projectId?:string; initialAgentSlug?:string }) {
  ...
  return <FileWorkspaceProvider key={`${user?.user_id ?? "pending"}:${displayedCurrent?.id ?? "empty"}`} conversationId={displayedCurrent?.id} storageKey={user && displayedCurrent ? viewKey(user.user_id, user.workspace_id, displayedCurrent.id) : ""} events={eventsByConversation[displayedCurrent?.id ?? ""] ?? []} runId={runStatus.conversationId === conversationId ? runStatus.runId : undefined} running={runState === "running" || runState === "sending" || runState === "stopping"}><main className={`workspace-shell ${renderedSidebarCollapsed ? "sidebar-collapsed" : ""} ${panelResize ? "panel-resizing" : ""}`} style={shellStyle}>
    ...
    <section className={`conversation-main ${displayedCurrent ? "" : "empty-conversation"}`}>
      <header className="conversation-header"><div className="conversation-header-leading"><button className="icon-button mobile-menu" onClick={() => setMobileMenu(true)} aria-label="打开会话栏"><Menu /></button><button type="button" className="sidebar-restore-button" onClick={() => setConversationSidebar(false)} title="展开会话栏" aria-label="展开会话栏"><PanelLeftOpen /></button>{displayedCurrent ? <div className="agent-heading"><span className="agent-avatar">{agentName(displayedCurrent.agent_slug,displayedCurrent.agent_name)[0]}</span><span><strong>{agentName(displayedCurrent.agent_slug,displayedCurrent.agent_name)}</strong><small className={`run-state ${runState}`}>{activeLabel}</small></span></div> : <Brand />}</div>{displayedCurrent ? <label className="model-selector"><select aria-label="当前模型" value={displayedCurrent.model_deployment_id || ""} onChange={(event) => chooseModel(event.target.value)}>{deployments.length === 0 ? <option value="">请先配置模型</option> : null}{deployments.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><ChevronDown /></label> : <button className="new-button mobile-new" onClick={() => { setMobileMenu(false); router.push(newConversationPath); }}><Plus />新会话</button>}<Link className="workspace-artifacts-link" href={`/app/artifacts?returnTo=${encodeURIComponent(conversationId ? `/app/c/${conversationId}` : newConversationPath)}`} aria-label="打开产物管理" title="产物管理"><ArtifactIcon size={18}/><span>产物管理</span></Link>{displayedCurrent?.created_agent_id ? <OpenAgentButton /> : null}{displayedCurrent ? <OpenFilesButton /> : null}</header>
...
function OpenAgentButton() {
  ...
  return <button type="button" className="open-agent-button" onClick={() => { setPanelTab("agent"); setPanelOpen(true); }}><Bot size={16} />智能体</button>;
}
...
function ComputerPanel({ conversationId, createdAgentId, agentRevision, width, maxWidth, resizing, onResizeStart, onWidthChange }: { conversationId: string; createdAgentId?: string; agentRevision: number; width: number; maxWidth: number; resizing: boolean; onResizeStart: (clientX: number) => void; onWidthChange: (width: number) => void }) {
  ...
  return <aside className={`computer-panel ${expanded ? "preview-expanded" : ""} ${panelOpen ? "files-open" : ""}`}><button type="button" className={`panel-resizer ${resizing ? "active" : ""}`} onPointerDown={(event) => { event.preventDefault(); onResizeStart(event.clientX); }} onKeyDown={(event) => { if (event.key === "ArrowLeft") onWidthChange(width + 24); if (event.key === "ArrowRight") onWidthChange(width - 24); }} role="separator" aria-label="调整 Computer 面板宽度" aria-orientation="vertical" aria-valuemin={minPanelWidth} aria-valuemax={Math.round(maxWidth)} aria-valuenow={Math.round(width)} title="拖动调整面板宽度"><GripVertical /></button><header className="computer-panel-header">
      <nav className="computer-tabs" aria-label="工作区功能">
        ...
        {createdAgentId ? <button aria-pressed={tab === "agent"} className={tab === "agent" ? "active" : ""} onClick={() => setTab("agent")}><Bot />智能体</button> : null}
      </nav>
