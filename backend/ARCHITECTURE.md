...
`read` uses the Sandbox Service's streaming line-range endpoint rather than loading an entire text file into API memory. It uses cat-n-style line prefixes and bounded contiguous pages, with an exact `next_offset`. Very long lines are individually marked truncated; their remainder requires a narrower inspection. File content/indentation is never rewritten by numbering. Image extensions are read through the bounded raw-file API and returned as an 8 MiB maximum base64 `images[]` envelope; model integrations map that envelope to native vision content parts (OpenAI-compatible providers receive a companion image user part, while Anthropic-compatible providers receive image blocks in `tool_result`). Raw file access is capped at 25 MiB, directory listings at 500 entries, and command stdout/stderr at 256 KiB per stream before the model-facing character limit is applied.
