...
func (c *Client) ListFiles(ctx context.Context, id, path string) ([]FileEntry, error) {
	var result struct{ Files []FileEntry }
	err := c.json(ctx, "GET", "/v1/sandboxes/"+id+"/files?path="+url.QueryEscape(path), nil, &result)
	return result.Files, err
}
func (c *Client) ReadFile(ctx context.Context, id, path string) ([]byte, error) {
	request, _ := http.NewRequestWithContext(ctx, "GET", c.BaseURL+"/v1/sandboxes/"+id+"/files/content?path="+url.QueryEscape(path), nil)
...
func (c *Client) WriteFile(ctx context.Context, id, path string, data []byte) error {
	request, _ := http.NewRequestWithContext(ctx, "PUT", c.BaseURL+"/v1/sandboxes/"+id+"/files/content?path="+url.QueryEscape(path), bytes.NewReader(data))
	response, err := c.HTTP.Do(request)
	if err != nil {
		return err
	}
