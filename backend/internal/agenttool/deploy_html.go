...
func (DeployHTML) Definition() model.Tool {
	return model.Tool{Name: "deploy_html", Description: "Publish a static HTML site and its local images, video, CSS and JS from this conversation to durable hosting. Returns a public URL accessible by anyone with the link. Use only when the user asks to deploy, publish, share, or obtain an external link; creating a file alone is not a publication request. A single HTML file publishes its static dependency closure; a directory publishes its static files with index.html by default. Never include secrets. Files max 25 MiB each, site max 100 MiB / 256 files. Supply artifact_id to replace your existing deployment without changing its URL. Explicit references outside the selected site directory are also collected when they remain inside this conversation, including .agent/upload attachments. Dynamic resource paths should use a complete directory and relative URLs. A loopback URL is reachable on the host computer, not from inside the sandbox; do not invent or change the hosting origin.", InputSchema: map[string]any{"type": "object", "properties": map[string]any{
