...
- The selected Agent is fixed for the lifetime of a conversation. Custom Agent names, instructions and selected Skill slugs are snapshotted at conversation creation; edits or deletion affect only future conversations. Default new conversations use Lester. The dedicated Agent builder can suggest a draft but only explicit user save creates or updates an Agent.
